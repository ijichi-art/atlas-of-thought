...
export function CityDetailPanel({
  ...
  // Reset chat state when a different city is opened.
  useEffect(() => {
    setChatMode(false);
    setChatMessages([]);
    setChatError(null);
  ...
  async function handleSend() {
    ...
    // Seed Claude's context with the city's JSON messages, then the chat history.
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 401) {
          ...
          setChatError("Add your Anthropic API key in Settings first.");
        } else {
          setChatError((data as { message?: string }).message ?? "Something went wrong.");
        }
        setIsStreaming(false);
        return;
      }
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5 min-h-0">
            {!chatMode ? (
              ...
              <ul className="space-y-2.5">
                {/* Seed: collapsed preview of existing messages */}
                {city.messages && city.messages.length > 0 && (
